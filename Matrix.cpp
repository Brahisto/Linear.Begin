...
void Matrix::operator*(const float v) noexcept {
    for (auto& i : matrix_) i *= v;
}
    ...
    if (type_figure == matrix_type::SQUERE) {
        ...
        for (int g{}; g < current.get_size();g++) {
            for (int h{}; h < current.get_size(); h++) {
                if (h < j and g < i) *(current.matrix_.begin() + (n_-1)*g + h) = this->operator()(g, h);
                else if (h < j and g >= i) *(current.matrix_.begin() + (n_-1)*g + h) = this->operator()(g+1, h);
                else if (h >= j and g < i) *(current.matrix_.begin() + (n_-1)*g + h) = this->operator()(g, h+1);
                else if (h >= j and g >= i) *(current.matrix_.begin() + (n_-1)*g + h) = this->operator()(g+1, h+1);
            }
        }
        return current;
    }
...
float scalar_product(const std::vector<float> & x, const Matrix& G, const std::vector<float> & y) {  //invoking matrix as Gram's matrix.
...
void Matrix::orthogonalization(const Matrix & G) {
    std::vector<vector_scalar_quad> basis (m_, {m_, 0});
    std::vector<float> a_i (m_, 0);
    basis[0].vector.coordinates.assign(matrix_.begin(), matrix_.begin() + m_);
    basis[0].scalar_quadrat = scalar_product(basis[0].vector.coordinates, G, basis[0].vector.coordinates);

    for (int i{1}; i < n_; i++) {
        a_i.assign(matrix_.begin() + m_*i, matrix_.begin() + m_*i + m_);
        basis[i].vector.coordinates.assign(matrix_.begin() + m_*i, matrix_.begin() + m_*i + m_);
        for (int j{}; j < i; j++) {
            basis[i].vector = basis[i].vector - basis[j].vector*(scalar_product(a_i, G, basis[j].vector.coordinates)/basis[j].scalar_quadrat);
        }
        basis[i].scalar_quadrat = scalar_product(basis[i].vector.coordinates, G, basis[i].vector.coordinates);
    }

    for (int i{}; i < n_; i++) {
        for (int j{}; j < m_; j++) {
            *(matrix_.begin() + m_*i + j) = *(basis[i].vector.coordinates.begin() + j);
        }
    }
}


float Matrix::ort_check(int i, int j, const Matrix & G) {
    float c{1};
    std::vector<float> vec1(matrix_.begin() + m_*(i-1), matrix_.begin() + m_*(i-1) + m_);
    std::vector<float> vec2(matrix_.begin() + m_*(j-1), matrix_.begin() + m_*(j-1) + m_);
    c = scalar_product(vec1, G, vec2);
